...
export function CatalogClient({ products }: { products: Product[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return products
      .filter((p) => !term || [p.brand, p.model, p.storage].some((value) => value.toLowerCase().includes(term)))
  ...
  return (
    <>
      <section className="catalog-hero"><div className="container"><span className="eyebrow">Catálogo demonstrativo</span><h1>Encontre seu próximo celular</h1><p>Pesquise entre os aparelhos disponíveis e fale com a equipe pelo WhatsApp.</p><div className="search-box"><Search /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Pesquise por modelo, marca ou armazenamento..." aria-label="Pesquisar celulares" /></div></div></section>
      <div className="container catalog-layout">
        <aside className={`filters-panel ${filtersOpen ? "mobile-open" : ""}`}>{filters}</aside>
        <section>
          <div className="catalog-topbar"><div><strong>{filtered.length} {filtered.length === 1 ? "celular encontrado" : "celulares encontrados"}</strong><button className="button button-soft mobile-filter" onClick={() => setFiltersOpen(true)}><Filter size={17} /> Filtros</button></div><select className="sort-select" value={sort} onChange={(e) => setSort(e.target.value as Sort)} aria-label="Ordenar celulares"><option value="recent">Mais recentes</option><option value="low">Menor preço</option><option value="high">Maior preço</option><option value="az">A–Z</option></select></div>
          <div className="products-grid catalog-products">{filtered.length ? filtered.map((product) => <ProductCard key={product.id} product={product} />) : <div className="empty-state"><SlidersHorizontal size={42} /><h3>Nenhum celular encontrado</h3><p>Tente remover algum filtro ou buscar outro modelo.</p><button className="button button-soft" onClick={clear}>Limpar filtros</button></div>}</div>
